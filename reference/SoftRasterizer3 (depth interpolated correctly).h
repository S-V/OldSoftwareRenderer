...
void TemplateRasterizeTriangle( const XVertex& v1, const XVertex& v2, const XVertex& v3, const SoftRenderContext& context )
{
	...
	*/
	...
	const INT32 nMinX = Clamp( (Min3(X1, X2, X3) + 0xF) >> 4, 0, W );
	const INT32 nMaxX = Clamp( (Max3(X1, X2, X3) + 0xF) >> 4, 0, W );
	const INT32 nMinY = Clamp( (Min3(Y1, Y2, Y3) + 0xF) >> 4, 0, H );
		...
		for( int iX = nMinX; iX < nMaxX; iX++ )
		{
			...
			//if( CX1 < 0 && CX2 < 0 && CX3 < 0 )
			{
				...
				if ( z < prevDepth )
				{
					...
					//const FColor c1 = FColor::YELLOW;
					//const FColor c2 = FColor::GREEN;
					//const FColor c3 = FColor::RED;
					//pixels[ iX ] = (c1 * t1 + c2 * t2 + c3 * t3).ToARGB().AsUInt32();

					//// visualize normals
					//FColor	c;
					//c.mSimdQuad = (v1.N * t1 + v2.N * t2 + v3.N * t3).quad;
					//pixels[ iX ] = c.ToARGB().AsUInt32();
