			...
			else // Partially covered block
			{
				...
				for( UINT iY = iBlockY; iY < iBlockY + BLOCK_SIZE_Y; iY++ )
				{
					...
					for( UINT iX = iBlockX; iX < iBlockX + BLOCK_SIZE_X; iX += SSE_REG_WIDTH )
					{
						...
						__m128i result = _mm_or_si128( oldQuad, _mm_and_si128( _mm_set1_epi32(RGBA8_WHITE), cx_mask_comp ) );

						_mm_store_si128( dest, result );

						//if( mask & 1 )	pixels[ iX + 0 ] = RGBA8_WHITE;
						//if( mask & 2 )	pixels[ iX + 1 ] = RGBA8_WHITE;
						//if( mask & 4 )	pixels[ iX + 2 ] = RGBA8_WHITE;
						//if( mask & 8 )	pixels[ iX + 3 ] = RGBA8_WHITE;
