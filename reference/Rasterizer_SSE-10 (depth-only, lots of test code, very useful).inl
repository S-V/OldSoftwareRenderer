						   ...
						   )
{
	...
	const ARGB32 COLOR = RGBA8_WHITE;
	...
	const INT32 X1 = iround( F4(1<<SHIFT) * fX1 );
	const INT32 X2 = iround( F4(1<<SHIFT) * fX2 );
	const INT32 X3 = iround( F4(1<<SHIFT) * fX3 );

	const INT32 Y1 = iround( F4(1<<SHIFT) * fY1 );
	const INT32 Y2 = iround( F4(1<<SHIFT) * fY2 );
	const INT32 Y3 = iround( F4(1<<SHIFT) * fY3 );
...
#endif
			{
				...
				for( UINT iY = iBlockY; iY < iBlockY + BLOCK_SIZE_Y; iY++ )
				{
					...
					for( UINT iX = iBlockX; iX < iBlockX + BLOCK_SIZE_X; iX += SSE_REG_WIDTH )
					{
						...
						const UINT32 nEdgeMask = _mm_movemask_ps( (__m128&)qiEdgeMask );
						for( UINT k=0; k<4; k++ )
						{
							if( nEdgeMask & (1<<k) )
							{
								// start value for x and y
								const F4 fX = (F4)(iX+k) - fX1;
								const F4 fY = (F4)iY - fY1;

								// interpolate depth
								const F4 Z = fZ1 + vZ.x * fX + vZ.y * fY;

								const ZBufElem thisDepth = FloatDepthToZBufferValue( Z );
								const ZBufElem prevDepth = zbuffer[ iX+k ];
								if( thisDepth < prevDepth )
								{
									// depth test passed
									zbuffer[ iX+k ] = thisDepth;

									pixels[ iX+k ] = SINGLE_FLOAT_TO_RGBA32( thisDepth );
								}
							}
						}
#endif
						...
						const UINT32 nEdgeMask = _mm_movemask_ps( (__m128&)qiEdgeMask );
						const F4* arrDepth = (const F4*) &qfZ;
						for( UINT k=0; k<4; k++ )
						{
							if( nEdgeMask & (1<<k) )
							{
								const ZBufElem thisDepth = FloatDepthToZBufferValue( arrDepth[k] );
								const ZBufElem prevDepth = zbuffer[ iX+k ];
								if( thisDepth < prevDepth )
								{
									// depth test passed
									...
									pixels[ iX+k ] = SINGLE_FLOAT_TO_RGBA32( arrDepth[k] );
								}
							}
						}
#endif
						...
						// depth test: if this depth quad is less or equal than previous depth quad, then draw these pixels
						const __m128i qiDepthMask = _mm_and_si128( (__m128i&) _mm_cmple_ps( qfZ, qfOldDepth ), qiEdgeMask );
						if( !_mm_movemask_ps( (__m128&)qiDepthMask) ) {
							goto L_Skip_This_Quad;
						}


						const UINT32 nMask = _mm_movemask_ps( (__m128&)qiDepthMask );
						const F4* arrDepth = (const F4*) &qfZ;
						for( UINT k=0; k<4; k++ )
						{
							if( nMask & (1<<k) )
							{
								zbuffer[ iX+k ] = arrDepth[k];
								pixels[ iX+k ] = SINGLE_FLOAT_TO_RGBA32( arrDepth[k] );
							}
						}
...
L_Skip_This_Quad:
						...
						const UINT32 nMask = _mm_movemask_ps( (__m128&)qiColorMask );
						const F4* arrDepth = (const F4*) &qfZ;
						for( UINT k=0; k<4; k++ )
						{
							if( nMask & (1<<k) )
						...
						__m128i qiNewColor = qiTmp;
			#else

						__m128i qiNewColor = (__m128i&)( qfZ );
			#endif
						...
						const UINT32 nMask = _mm_movemask_ps( (__m128&)qiColorMask );
						const F4* arrDepth = (const F4*) &qfZ;
						const U4* arrColor = (const U4*) &qiNewColor;
						for( UINT k=0; k<4; k++ )
						{
							if( nMask & (1<<k) )
							{
								//pixels[ iX+k ] = SINGLE_FLOAT_TO_RGBA32( arrDepth[k] );
								pixels[ iX+k ] = arrColor[k];
							}
						}
			#endif
